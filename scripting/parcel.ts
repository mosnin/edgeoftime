...
import { vec3, Vector3 } from './lib/maths'
...
export default class Parcel extends AbstractParcel {
  ...
  private onMessage(player: Player, msg: Message) {
    if (!msg.event) {
      msg.event = {}
    }
    ...
    if (msg.type === 'playerenter') {
      ...
      if (msg.event.point) {
        e.point = Vector3.FromArray(msg.event.point as vec3)
      }
      if (msg.event.normal) {
        e.normal = Vector3.FromArray(msg.event.normal as vec3)
      }
  ...
  private onCryptoHash = (msg: { hash: string; quantity: number; to: string; chain_id: number; erc20Address: string | undefined }) => {
  ...
  start() {
    if (this._initiated) {
      console.log('Parcel already initiated')
      return
    }
    this._initiated = true
    console.log('Parcel started')

    for (let f of this.featuresList) {
      if (f.hasScript) {
        // console.log('Attached script for feature', f.uuid)
        f.evalScript()
      }
    }
    ...
    this._messageHandler = (e) => {
      ...
      if (!Object.values(SupportedMessageTypes).includes(data.type)) {
        console.log('[Scripting] Invalid message type', data.type)
        // invalid message type, ignore
        return
      }

      // if (!data.player && !data.player._token) {
      //   console.log('No player record in the dataPacket', data.player)
      //   // no player record in the dataPacket, ignore
      //   return
      // }

      const token = e.data.player.token as string

      if (!token) {
        console.log('No token in the event', e)
        return
      }
      ...
      if (!player) {
        // console.log('Creating player', e.data.player)
        player = new Player(e.data.player, this)
    ...
    globalThis.onmessage = this._messageHandler
  }
      ...
      if (state) {
        ...
        this.players.forEach((player) => {
          if (!player.isLoggedIn()) {
            // Player not allowed
            player.kick(`Parcel ${this.id} switched to logged in only mode and you're not logged in.`)
          }
        })
      }
    } else {
  ...
  /**
   * Dispose of the parcel and clean up all resources to prevent memory leaks
   */
  dispose() {
    ...
    this.featuresList.forEach((feature) => {
      // Remove all event listeners from the feature
    ...
    // Remove all event listeners from the parcel itself
...
export class Space extends Parcel {
  ...
  /**
   * Override dispose to ensure Space-specific cleanup
   */
  dispose() {
    super.dispose()
    console.log('Space disposed')
  }
}
