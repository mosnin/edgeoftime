...
export class Player extends EventEmitter {
  ...
  constructor(description: PlayerDescription, parcel: Parcel) {
    ...
    // console.log('Creating player', description)

    this.parcel = parcel
    this.token = description.token as string
   ...
   * @returns
   */
  // hasEthereumNFT = (contract: string, tokenId: string | number, successCallback: ((bool: boolean) => void) | null = null, failCallback: ((reason: string) => void) | null = null) => {
  //   if (!this.isLoggedIn()) {
   ...
   * @returns
   */
  // hasNFT = (chain: string, contract: string, tokenId: string | number, successCallback: ((bool: boolean) => void) | null = null, failCallback: ((reason: string) => void) | null = null) => {
  //   if (!this.isLoggedIn()) {
  //     failCallback && failCallback('User is not logged in')
  //     return false
  //   }
  ...
  ///@private
  private _askForCrypto(quantity: number = 0.01, to: string = this.parcel.owner, chain_id: number = 1, erc20Address?: string) {
    this.parcel.broadcast({
  ...
  askForCrypto = throttle(
    (quantity: number = 0.01, chain_id: number = 1, to: string = this.parcel.owner, erc20Address?: string) => {
      this._askForCrypto(quantity, to, chain_id, erc20Address)
    },
    1500,
    { trailing: false, leading: true },
  ) as (quantity: number, chain_id?: number, to?: string, erc20Address?: string) => void
