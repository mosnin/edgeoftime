...
export default class CodeFlask {
  ...
  opts: any
  code: string = ''
  elWrapper: HTMLElement | null = null
  elTextarea: HTMLTextAreaElement | null = null
  elPre: HTMLElement | null = null
  elCode: HTMLElement | null = null
  elLineNumbers: HTMLElement | null = null
  lineNumber: number = 0
  updateCallBack: ((code: string) => void) | null = null

  constructor (selectorOrElement: HTMLElement | string, opts: any) {
    ...
    if (selectorOrElement instanceof HTMLElement) {
      ...
      if (editorRoot) {
        this.editorRoot = editorRoot as HTMLElement
      }
    }
  ...
  createTextarea () {
    this.elTextarea = this.createElement('textarea', this.elWrapper!) as HTMLTextAreaElement
  ...
  createElement (elementTag: string, whereToAppend: HTMLElement) {
  ...
  listenTextarea () {
    this.elTextarea!.addEventListener('input', (e) => {
      ...
      this.elCode!.innerHTML = escapeHtml((e.target as HTMLTextAreaElement).value)
      this.highlight()
      setTimeout(() => {
        this.runUpdate()
        this.setLineNumber()
    ...
    this.elTextarea!.addEventListener('scroll', (e) => {
      this.elPre!.style.transform = `translate3d(-${(e.target as HTMLTextAreaElement).scrollLeft}px, -${(e.target as HTMLTextAreaElement).scrollTop}px, 0)`
      if (this.elLineNumbers) {
        this.elLineNumbers!.style.transform = `translate3d(0, -${(e.target as HTMLTextAreaElement).scrollTop}px, 0)`
      }
    })
  }

  handleTabs (e: KeyboardEvent) {
  ...
  handleSelfClosingCharacters (e: KeyboardEvent) {
  ...
  handleNewLineIndentation (e: KeyboardEvent) {
  ...
  addLanguage (name: string, options: any) {
